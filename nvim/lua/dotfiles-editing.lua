...
---@param hex string
local function apply_color_template_hl(buf, lnum, start, finish, hex)
...
---@param start integer 1-based inclusive
---@param finish integer 1-based exclusive
local function apply_generic_template_hl(buf, lnum, start, finish)
    vim.hl.range(
        buf,
        ns,
        'DotfilesTemplate',
...
local function apply_highlights(buf, colors)
    ...
    for l, line in ipairs(lines) do
        for start, finish, template in dotter.find_template_matches(line) do
            ...
            if color_name and colors[color_name] then
                ...
                apply_generic_template_hl(buf, l - 1, start, finish)
            end
        end
    end
end
...
vim.api.nvim_create_autocmd({ 'BufRead', 'BufNewFile' }, {
    callback = function()
        ...
        dotter.load_colors_async(root, function(dotter_colors)
            ...
            apply_highlights(buf, dotter_colors)
        end)
    end,
})
...
vim.api.nvim_create_autocmd({ 'TextChanged', 'TextChangedI', 'TextChangedP' }, {
    callback = function()
        if vim.b.dotfiles_colormap then
            apply_highlights(0, vim.b.dotfiles_colormap)
        end
    end,
})
